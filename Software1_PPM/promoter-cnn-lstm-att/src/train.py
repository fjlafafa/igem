...
from model import EPT, CLAModel, make_convblock
...
class PromoterDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return torch.tensor(self.text[idx],dtype=torch.int32), torch.tensor(self.label[idx],dtype=torch.float32)
...
def train(Args):
    ...
    convblock_1 = make_convblock(in_channels=48, 
                                 ...
                                 conv_kernel_size=Args.kernel_1, 
                                 stride=Args.stride, 
                                 padding='valid',
                                 pool_kernel_size=Args.pool,
                                 dropout=Args.dropout)
    convblock_2 = make_convblock(in_channels=Args.filter, 
                                 out_channels=Args.filter, 
                                 conv_kernel_size=Args.kernel_2, 
                                 stride=Args.stride, 
                                 padding='valid',
                                 pool_kernel_size=Args.pool,
                                 dropout=Args.dropout)
    base_model = CLAModel(input_size=len(vocab),
                       embedding_size=Args.embedding,
                       dropout=Args.dropout,
                       convblock_1=convblock_1,
                       convblock_2=convblock_2,
                       lstm_input_size=Args.input,
                       lstm_hidden_size=Args.hidden,
