...
def load_data(data_path):
    """Load data."""
    # Load data
    promoter = pd.read_csv(data_path, header=None)
    promoter.columns = ['sequence']
    # Prepare data
    tokens = tokenize(promoter['sequence'])
    vocab = Vocab(tokens)
    vocab.token_to_idx.pop('<unk>')
    vocab.idx_to_token.pop(0)
    corpus = np.array([vocab[token] for token in tokens], dtype=np.int32)
    return promoter, corpus